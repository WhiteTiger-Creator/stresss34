#!/usr/bin/env python3
"""Repaired sample window compiler."""  # noqa: EXE001
...
def canonicalize(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        current = deduped.get(sample_id)
        if current is None:
            deduped[sample_id] = normalized
            continue
        replace = False
        if normalized["end_ms"] > current["end_ms"]:
            ...
            # MET-2258 reverses this: the LOWER-ranked severity wins a duplicate tie.
    ...
    return sorted(
        deduped.values(),
        key=lambda row: (row["service"], row["start_ms"], str(row["sample_id"])),
    )
...
def merge_windows(
    ...
    for service in sorted(by_service):
        ...
        for row in rows:
                ...
                merged.append(
                    {
                        ...
                        "end_ms": row["end_ms"],
                        "sample_count": 1,
                        "critical_sample_count": 1 if row["severity"] == "critical" else 0,
                        "source_sample_ids": [str(row["sample_id"])],
            ...
            if row["start_ms"] <= prev["end_ms"] + 30:
                ...
                if row["severity"] == "critical":
                    prev["critical_sample_count"] += 1
                sample_id = str(row["sample_id"])
                if sample_id not in prev["source_sample_ids"]:
                    prev["source_sample_ids"].append(sample_id)
        ...
        for block in merged:
            ...
            block["source_sample_ids"] = sorted(block["source_sample_ids"])
            ...
            block["adjusted_billable_duration_ms"] = max(
                # Handoff overlap subtracted ROUNDED UP per MET-2252. ceil(x/2)=-(-x//2).
            ...
            block["routed_billable_duration_ms"] = max(
                # Blackout overlap subtracted ROUNDED UP per MET-2254. ceil(x/3)=-(-x//3).
            ...
            block["window_signature"] = hashlib.sha1(
                (
                    ...
                    f"{','.join(block['source_sample_ids'])}|{block['max_severity']}|"
        ...
        for index, block in enumerate(merged):
            ...
            debt_adjusted_dispatchable_ms = (
                # Debt credit ROUNDS UP per MET-2242. ceil(x/5) = -(-x // 5).
...
def build_queue(
    ...
    for service, blocks in windows.items():
        ...
        for block in blocks:
            ...
            # Boost units counted ROUNDED UP per MET-2256, like the suppression unit.
            ...
            blackout_pressure_score = (
                ...
                # Severity-scoped blackout probe ROUNDS UP per MET-2246. ceil(x/24)=-(-x//24).
            ...
            debt_pressure_score = (
                ...
                # Debt-in term ROUNDS UP per MET-2248. ceil(x/120)=-(-x//120).
            ...
            escalation_score = (
                ...
                + block["sample_count"] * 2
                + block["critical_sample_count"] * 3
            ...
            if (
                ...
                or block["critical_sample_count"] >= policy["critical_count_for_critical"]
                ...
                or (block["sample_count"] >= 3 and block["max_severity"] in {"major", "critical"})
                or (
                ...
                priority = "medium"
            sample_ids_csv = ",".join(block["source_sample_ids"])
            signature = hashlib.sha1(
                (
                    f"{service}|{block['start_ms']}|{block['end_ms']}|"
                    f"{sample_ids_csv}|{block['window_signature']}|{block['max_severity']}|"
            ...
            rows.append(
                {
                    ...
                    "critical_sample_count": block["critical_sample_count"],
                    "source_sample_ids": block["source_sample_ids"],
...
def build_summary(
    ...
    canonical_input_checksum = hashlib.sha256(
        "\n".join(
            (
                f"{row['sample_id']}|{row['service']}|{row['start_ms']}|"
    ...
    queue_signature_checksum = hashlib.sha256(
        "|".join(row["sample_signature"] for row in queue).encode("utf-8")
    ).hexdigest()
    ...
    return {
        "schema_version": "rollup-windows-v1",
        "raw_sample_count": len(raw_rows),
        "unique_sample_ids": len({str(row["sample_id"]) for row in raw_rows}),
...
def write_outputs(
    ...
    (output_dir / "rollup_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    (output_dir / "gap_windows.json").write_text(json.dumps(windows, indent=2) + "\n")
    with (output_dir / "alert_queue.jsonl").open("w", encoding="utf-8") as handle:
        for row in queue:
            handle.write(json.dumps(row, separators=(",", ":")) + "\n")


def metric_rollup(input_path: Path, output_dir: Path) -> None:
    raw_rows = load_samples(input_path)
...
def main() -> None:
    ...
    parser.add_argument("--input", default="/app/data/samples.json")
    parser.add_argument("--output-dir", default="/app/output")
    args = parser.parse_args()

    metric_rollup(input_path=Path(args.input), output_dir=Path(args.output_dir))
