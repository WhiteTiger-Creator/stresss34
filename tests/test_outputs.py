"""Verifier tests for sample compiler hard task."""
...
SUMMARY_PATH = OUTPUT_DIR / "rollup_summary.json"
WINDOWS_PATH = OUTPUT_DIR / "gap_windows.json"
QUEUE_PATH = OUTPUT_DIR / "alert_queue.jsonl"

PIPELINE = Path("/app/workflow/metric_rollup.py")
ORIGINAL_PIPELINE = Path("/app/workflow/.metric_rollup.original")
INPUT_PATH = Path("/app/data/samples.json")
MAINTENANCE_PATH = Path("/app/data/maintenance_windows.json")
POLICY_PATH = Path("/app/data/response_policies.json")
EXCEPTIONS_PATH = Path("/app/data/rollup_exceptions.json")
...
CONTRACT_PATH = Path("/app/docs/rollup_contract.json")
ALT_INPUT_PATH = Path("/tests/fixtures/alt_samples.json")

FIXTURE = json.loads(Path("/tests/fixtures/expected_outputs.json").read_text())
CONTRACT = json.loads(CONTRACT_PATH.read_text())
BROKEN_PIPELINE_SHA256 = "1b80ee99e00c79486a0ede132aadf4e5589be92bd5d5d1486f8259006e66af7a"
...
def test_identifier_contract_vectors():
    ...
    assert (
        hashlib.sha1(vectors["sample_signature_payload"].encode("utf-8")).hexdigest()[:12]
        == vectors["sample_signature"]
    )
    assert (
...
def test_cli_exists():
    """The sample compiler exists at its operational path."""
...
def test_output_dir_contains_exactly_three_files():
    ...
    assert names == {"rollup_summary.json", "gap_windows.json", "alert_queue.jsonl"}


def test_primary_summary_exact_fixture(summary: dict):
    """rollup_summary.json from the primary input matches its canonical SHA-256 exactly."""
...
def test_primary_windows_exact_fixture(windows: dict[str, list[dict]]):
    """gap_windows.json from the primary input matches its canonical SHA-256 exactly."""
    assert _stable_json_hash(windows) == FIXTURE["primary_windows_sha256"]


def test_primary_queue_exact_fixture(queue_rows: list[dict]):
    """alert_queue.jsonl from the primary input matches its canonical SHA-256 exactly."""
...
def test_broken_snapshot_is_wrong():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        broken = Path(tmp) / "metric_rollup.py"
        out = Path(tmp) / "broken_out"
        shutil.copy(ORIGINAL_PIPELINE, broken)
        result = _run_pipeline(pipeline=broken, output_dir=out)
        assert result.returncode == 0, result.stderr
        broken_summary = _load_json(out / "rollup_summary.json")
        broken_windows = _load_json(out / "gap_windows.json")
...
def test_pipeline_rerun_idempotent(summary: dict, windows: dict[str, list[dict]], queue_rows: list[dict]):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert result.returncode == 0, result.stderr
        assert _load_json(rerun / "rollup_summary.json") == summary
        assert _load_json(rerun / "gap_windows.json") == windows
        assert _load_jsonl(rerun / "alert_queue.jsonl") == queue_rows
...
def test_pipeline_supports_custom_output_dir():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert files == {"rollup_summary.json", "gap_windows.json", "alert_queue.jsonl"}
...
def test_cli_defaults_work_and_match_explicit_run():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert _load_json(SUMMARY_PATH) == _load_json(explicit_out / "rollup_summary.json")
        assert _load_json(WINDOWS_PATH) == _load_json(explicit_out / "gap_windows.json")
        assert _load_jsonl(QUEUE_PATH) == _load_jsonl(explicit_out / "alert_queue.jsonl")
...
def test_policy_source_path_affects_output():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary_a = _load_json(out_a / "rollup_summary.json")
            queue_a = _load_jsonl(out_a / "alert_queue.jsonl")
            ...
            summary_b = _load_json(out_b / "rollup_summary.json")
            queue_b = _load_jsonl(out_b / "alert_queue.jsonl")
...
def test_maintenance_compaction_and_span_count_exercised():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert result.returncode == 0, result.stderr
            windows = _load_json(out / "gap_windows.json")
...
def test_dedupe_tie_break_chain_exercised():
    """Duplicate sample_id rows collapse to one row via the full governed tie-break chain.
    ...
    try:
        MAINTENANCE_PATH.write_text("[]\n")
        rows = [
            {"sample_id": "d1", "service": "payments", "start_ms": 100, "end_ms": 200, "severity": "major", "planned": True},
            {"sample_id": "d1", "service": "billing", "start_ms": 120, "end_ms": 200, "severity": "major", "planned": False},
            {"sample_id": "d1", "service": "auth", "start_ms": 130, "end_ms": 200, "severity": "critical", "planned": False},
            {"sample_id": "d1", "service": "billing", "start_ms": 130, "end_ms": 200, "severity": "critical", "planned": False},
        ]
        with tempfile.TemporaryDirectory() as tmp:
            in_path = Path(tmp) / "in.json"
            out = Path(tmp) / "out"
            ...
            block = windows["billing"][0]
            # MET-2258 reverses the severity step: the LOWER rank wins the tie, so the
            ...
            assert block["source_sample_ids"] == ["d1"]
    finally:
        MAINTENANCE_PATH.write_text(original)
...
def test_planned_coercion_aliases_and_unknown_severity_exercised():
    ...
    try:
        MAINTENANCE_PATH.write_text("[]\n")
        rows = [
            {"sample_id": "p1", "service": "authentication", "start_ms": 10, "end_ms": 300, "severity": "major", "planned": "no"},
            {"sample_id": "p2", "service": "search-api", "start_ms": 20, "end_ms": 260, "severity": "weird", "planned": "yes"},
            {"sample_id": "p3", "service": "payments", "start_ms": 30, "end_ms": 280, "severity": "critical", "planned": 2},
        ]
        with tempfile.TemporaryDirectory() as tmp:
            in_path = Path(tmp) / "in.json"
            out = Path(tmp) / "out"
...
def test_handoff_compaction_and_scope_exercised():
    """Handoff windows compact per (service, severity scope) and match only in-scope samples.

    A window attenuates an sample only when its severity scope covers that sample, and the
...
def test_blackout_compaction_and_scope_exercised():
    """Blackout windows compact per (service, severity scope) and match only in-scope samples.
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # adjusted - ceil(190/3) = adjusted - 64 per MET-2254 (floor gave 237)
...
def test_degrade_compaction_and_scope_exercised():
    """Degrade windows compact per (service, severity scope) and match only in-scope samples.
...
def test_debt_ledger_propagates_and_decays_between_windows():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # debt_out//80 + ceil(debt_in/120) = 8 + 4 per MET-2248 (floored debt_in gave 11)
...
CRON_LINE = (
    "*/5 * * * * svc-metrics /usr/local/bin/rollup-metrics "
    "--input /app/data/samples.json --output-dir /app/output"
)


def test_service_account_provisioned():
    """The svc-metrics system account exists with a non-interactive shell."""
    import pwd
...
def test_output_dir_ownership_and_mode():
    """/app/output is owned by svc-metrics with mode 0750."""
    import pwd
...
def test_governing_entry_index_is_complete():
    ...
    nonnorm = set(re.findall(r"MET-\d+", gi.get("non_normative_entries", "")))
    log_text = Path("/app/sample/rollup_review_log.md").read_text(encoding="utf-8")
    pattern = re.compile(r"\*\*[A-Za-z -]+? \(\d{4}-\d{2}-\d{2} - (MET-\d+)\)\*\*([^\n]*)")
    governing = {
        m.group(1)
...
def test_log_directory_ownership_and_mode():
    """/var/log/metrics-rollup is handed to the service account and is not world-writable.

    The rollout left the compile log directory root-owned at mode 0777. Recovery must place it
    under svc-metrics:svc-metrics at mode 0750, since the scheduled compile writes there as that
    ...
    assert info.st_uid == svc.pw_uid and info.st_gid == svc.pw_gid, "log dir not owned by svc-metrics"
...
def test_rollout_leftover_log_pruned():
    ...
    /var/log/metrics-rollup/compile.log.0 was left behind mid-compile and must be pruned during
    recovery rather than handed to the next rotation. The live compile.log is not a leftover and
    must remain in place.
    """
    assert not Path("/var/log/metrics-rollup/compile.log.0").exists(), "rollout leftover not pruned"
    assert Path("/var/log/metrics-rollup/compile.log").exists(), "live compile log was removed"


def test_logrotate_dropin_installed():
    """Rotation is configured to run as the service account, not as root.

    The drop-in at /etc/logrotate.d/metrics-rollup must exist at mode 0644 covering the compile
